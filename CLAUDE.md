...
- **`min-width: 0` va messo su OGNI livello annidato di flexbox che deve potersi restringere, non solo sull'ultimo.** Un flex item con `flex: 1` ha di default `min-width: auto`, quindi rifiuta di restringersi sotto la larghezza del proprio contenuto — anche se un suo FIGLIO ha gia' `min-width: 0`. Bug reale trovato in `MiniBarChart` (grafico "mese", 31 colonne): `.bar-col` aveva `min-width: 0` ma il suo genitore `.bar-chart-plot` (anch'esso `flex: 1`, dentro `.bar-chart` insieme all'asse Y) no, quindi l'intero grafico sporgeva fuori dal pannello su schermi stretti invece di restringersi. Quando un contenitore flex con molte colonne/elementi non si adatta a schermi stretti, controllare `min-width: 0` su OGNI antenato flex diretto fino al contenitore che ha la larghezza vincolata, non solo sulle colonne piu' interne.
